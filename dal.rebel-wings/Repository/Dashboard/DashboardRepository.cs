...
public class DashboardRepository : GenericRepository<biz.rebel_wings.Entities.SatisfactionSurvey>, IDashboardRepository
{
    ...
    public DashboardSupervisor GetSupervisorsV2(int id, DateTime dateStart, DateTime dateEnd, int isDone, int city)
    {
        ...
        taskPerShiftsList.AddRange(taskPerShiftsMorningList);
        taskPerShiftsList.AddRange(taskPerShiftsEveningList);
        var all = taskPerShiftsList.GroupBy(g=>g.NameTask).Select(grp => new{
                number  = grp.Key,
                nameTask = grp.First().NameTask,
        ...
        {
            dashboardSupervisor.OmissionsActivities += Decimal.Divide(task.NoComplete * percentageByTask, 100);
        }
