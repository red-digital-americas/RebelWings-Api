...
namespace dal.rebel_wings.Repository.Generic
{
    public abstract class GenericRepository<T> : IGenericRepository<T> where T : class
    {
        protected Db_Rebel_WingsContext _context;

        public GenericRepository(Db_Rebel_WingsContext context)
        {
            _context = context;
        }
        ...
        public virtual async Task<int> DeleteByAsync(Expression<Func<T, bool>> predicate)
        {
        ...
        {
            var count = await _context.Set<T>().Where(predicate).CountAsync();
            return count;
        }
        ...
        public decimal ProgressTask(int task, int total)
        {
            decimal percentageTotal = 100;
            return Convert.ToDecimal(percentageTotal * task) / total;
        }
    }
}
